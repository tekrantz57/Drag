#!/usr/bin/env python3
"""Loopback-only native speech bridge for DragWin."""
...
class PiperSpeechEngine:
    def __init__(self, data_dir: str):
        try:
            ...
            raise RuntimeError(
                "Piper is not installed; run: python -m pip install piper-tts"
            ) from error
    ...
    def _play_wav(audio: bytes) -> None:
        ...
        selected = next(
            ((shutil.which(name), args) for name, args in players if shutil.which(name)),
            None,
        )
        if selected is None:
            raise RuntimeError(
                "No supported audio player found (pw-play, paplay, aplay, or ffplay)"
            )
...
def default_piper_data_dir() -> str:
    configured = os.environ.get("DRAG_PIPER_VOICE_DIR")
    if configured:
        return configured
    if os.name == "nt":
        ...
        return str(Path(local_app_data) / "Drag" / "PiperVoices")
    xdg_data_home = os.environ.get(
        "XDG_DATA_HOME",
        str(Path.home() / ".local" / "share"),
    )
    return str(Path(xdg_data_home) / "Drag" / "PiperVoices")
...
def main() -> None:
    ...
    with SpeechServer(("127.0.0.1", args.port), engine) as server:
        print(
            f"Drag {args.engine} speech helper listening on 127.0.0.1:{args.port}",
            flush=True,
        )
